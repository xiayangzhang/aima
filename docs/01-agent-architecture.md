...
│  │  脑区之间不直接互相调用，通过 Thread Runner 路由（见协调层）                    │   │
│  └────────────────────────────────────────────────────────────────────────────── ┘   │
│                                          │                                            │
│  ┌─ 协调层 ──────────────────────────────┼───────────────────────────────────────┐   │
│  │  Thread Runner                        │   Context Assembly                     │   │
│  │  · 读 Slot 状态决定下一步激活哪个脑区  │   · 脑区激活前注入 Block 1–4           │   │
│  │  · 管理 pending_observations 成熟路由  │   · Block 4 调用 Hippocampus.Recall    │   │
│  │                                       │                                         │  │
│  │  认知工作空间（Thread / Slot / Signal，持久化 PostgreSQL）                       │  │
│  └─────────────────────────────────────────────────────────────────────────────── ┘  │
│                                          │                                            │
│  ┌─ 横切层 ──────────────────────────────────────────────────────────────────────┐   │
│  │  Brain Event Bus（所有脑区行为的结构化流）                                      │   │
