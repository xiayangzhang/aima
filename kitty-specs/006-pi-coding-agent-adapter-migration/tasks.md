...
- [ ] T010 `tool_execution_end` → emit `tool.post_use` (INFO) with `brain`, `thread_id`, `tool`, `toolCallId`, `isError`
...
### Included Subtasks
- [ ] T012 Find `createAimaMcpServer` export path in `src/`; implement `buildMcpTools(workspace): AgentTool[]` in `mcp-tools.ts`
