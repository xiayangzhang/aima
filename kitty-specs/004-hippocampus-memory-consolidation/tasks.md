...
- [x] T010: 执行合并更新：merge=true 时将第二段所有 episodic 记录 segment_id 改为第一段，重新编号 segment_seq
- [x] T011: 幂等保护：in-memory Set 跟踪本次已更新的段，防止重复合并
- [x] T012: 错误处理：单对合并失败不中断整体，warn 日志继续下一对；在 runConsolidation 中注册为步骤 1
...
- [x] T015: LLM 调用 + parseLlmJson：解析 semantic/procedural/implicit 数组；解析失败跳过该段
- [x] T016: 写入 semantic + procedural 记忆：supersedesId 去重（searchMemory 先查，存在时设 supersedesId + invalidateMemory 旧记录）
- [x] T017: 写入 implicit 记忆 + 错误处理：base_importance = 段 avgImportance × 0.8；单段失败不中断；在 runConsolidation 注册为步骤 2
...
- [x] T020: runExpiryCleanup(workspace, config)：forgetExpiredMemories(now) → 日志记录数量；在 runConsolidation 注册为步骤 3 + 步骤 4
- [x] T021: 顺序约束验证：runConsolidation 中步骤 1-4 按顺序 await，步骤 N 抛出时后续不执行；幂等文档注释
...
- [x] T027: converge-cleanup.test.ts Part C：runExpiryCleanup mock 验证 + 顺序约束（步骤 1 抛出 → 步骤 2 未执行，通过 spy 验证）
